        ...
        public void Edit()
        {
            ...
            else
            {
                try
                {
                    ...
                    SqlConnection con = new SqlConnection(connectionString);
                    con.Open();
                    string query = @"INSERT INTO  paysheet(paysheet_id,Emp_ID,Emp_Month,Emp_work_days,Emp_salary)
                        VALUES('"+txtpayid.Text+"','" + txt_emp_ID.Text + "','"+comb_pay_month.SelectedItem+"','" + txtEmp_workeddays.Text + "','" + txtsalary.Text + "')";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.ExecuteNonQuery();
